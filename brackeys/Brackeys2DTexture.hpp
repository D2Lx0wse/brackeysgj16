...
public:
	...
	bool render(float x, float y, Vector2 scalingSize = Vector2{ 0.0f, 0.0f }, float rotation = 0.0f, Color tint = WHITE) const;

	// Renders the texture to Vector2 coordinates. 
	//  - Optionally can choose the width/height seen on screen
	//  - Optionally can choose the degrees of rotation (counter-clockwise).
	// Returns true/false based on if texture is loaded or not
	bool render(Vector2 pos, Vector2 scalingSize = Vector2{ 0.0f, 0.0f }, float rotation = 0.0f, Color tint = WHITE) const;
