...
void CreditsScene::init() {
	*this = CreditsScene{};
	...
	m_creditsTitle.setColor(DARKBLUE);
	m_creditsTitle.setText("CREDITS", m_font); 
	
	m_credits = { { "Game done by", m_font }, { "Hypah and D2_Lx0wse", m_font }, { "With RayLib", m_font }, { "Random.h library", m_font }, { "from learncpp.com", m_font } };

	m_goBackText.setText("Go back to main menu", m_font);


	m_creditsTitle.setPosition(Vector2{ (m_sceneWidth - m_creditsTitle.textSize().x) / 2.0f, (m_sceneHeight / 100.0f) * 7.5f });

	constexpr float creditsFontSize{ Constants::g_FontSize64 / 1.25f };
	float verticalSpaceBetweenCredits{ creditsFontSize / 6.0f };
	for (auto& credit : m_credits) {
		credit.setFontSize(creditsFontSize, m_font);
		credit.setColor(BLUE);
		credit.setPosition(
			Vector2{ (Constants::g_ScreenWidth - credit.textSize().x) / 2.0f, (Constants::g_ScreenHeight - credit.textSize().y) / 2.75f + verticalSpaceBetweenCredits }
		);

		verticalSpaceBetweenCredits += credit.fontSize();
	}

	m_goBackText.setPosition(Vector2{ (m_sceneWidth - m_goBackText.textSize().x) / 2.0f, m_sceneHeight - m_goBackText.textSize().y - (m_sceneHeight / 100.0f) * 5.0f });
